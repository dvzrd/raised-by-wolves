...
import { Button, Card, Hero, Loader, Modal, Section } from "../../components";
...
export const LandingPage = () => {
  ...
  const [isModalPilotOpen, setModalPilotOpen] = useState(false);
  const [isModalTrailerOpen, setModalTrailerOpen] = useState(false);
  ...
  return (
    <>
      <DefaultLayout {...layoutProps}>
          ...
          actions={[
            {
              ...
              onClick: () => setModalPilotOpen(true),
            },
          ]}
          heading={show?.name}
        ...
        >
          <figcaption className="mb-4 xl:mb-6 mt-16 md:mt-24 xl:mt-32 font-semibold uppercase tracking-widest text-center sm:text-left">
            <h4 className="md:text-lg xl:text-xl text-gray-800">
              {show?.name}
            </h4>
            {featuredEpisode && (
      ...
      <Modal
        closeModal={() => setModalPilotOpen(false)}
        isOpen={isModalPilotOpen}
        videoId="YIAIiw8UAfA"
      />
      <Modal
        closeModal={() => setModalTrailerOpen(false)}
